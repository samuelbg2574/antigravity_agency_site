...
// Only set the iframe src once the card is near the viewport.
// Prevents external sites from loading on page load and tanking LCP.
function useLazyIframe(rootMargin = "400px") {
...
function FeaturedCard({ project }: { project: Project }) {
  const { wrapperRef, ready } = useLazyIframe();
  return (
    <div ref={wrapperRef}>
      <motion.div
        ...
        <Link href={project.href} className="group block">
          ...
          >
            ...
            <div className="absolute inset-0 transition-transform duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:scale-[1.03] origin-center">
              <iframe
                src={ready ? project.url : undefined}
                title={project.title}
